...
//    private final UserRepository userRepository;
    ...
    public ResponseEntity<List<User>> getUsers() {
        return ResponseEntity.ok().body(userServiceImplements.getUsers());
    }

    @PostMapping("/users/save")
    public ResponseEntity<User>saveUser(@RequestBody User user) {
        URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/teamD/v1/users/save").toUriString());
        return ResponseEntity.created(uri).body(userServiceImplements.createUser(user));
    }

    @PostMapping("/role/save")
    public ResponseEntity<Role>saveRole(@RequestBody Role role) {
        URI uri = URI.create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/teamD/v1/role/save").toUriString());
        return ResponseEntity.ok().body(userServiceImplements.saveRole(role));
    }
    ...
    public void refreshToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
            try{
                ...
                User user = userServiceImplements.getUser(username);
